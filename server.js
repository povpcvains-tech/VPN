...
app.get('/login', (req, res) => {
    const error = req.query.error === 'auth' ? '❌ Wrong login/password' : (req.query.error === 'blocked' ? '🚫 Account blocked' : '');
...
app.post('/login', (req, res) => {
    ...
    if (users[username] && bcrypt.compareSync(password, users[username].password)) {
        ...
        req.session.authenticated = true; req.session.userId = username;
        res.redirect('/');
    } else { res.redirect('/login?error=auth'); }
});
...
app.get('/change-password', isAuthenticated, (req, res) => {
    ...
    <input type="password" id="cur" placeholder="Current Password"><input type="password" id="newp" placeholder="New Password"><input type="password" id="conf" placeholder="Confirm New Password">
    <button onclick="submitPass()">Save</button><div class="link"><a href="/">Back</a></div></div>
    <script>async function submitPass(){
        ...
        if(n!==co)return document.getElementById('msg').innerHTML='<div class="error">Mismatch</div>';
        ...
        if(d.success){alert('Done');window.location.href='/';} else {document.getElementById('msg').innerHTML='<div class="error">'+d.error+'</div>';}
    }</script></body></html>`);
});
...
app.get('/', isAuthenticated, (req, res) => {
    if (!isDataLoaded) return res.send('<h1 style="color:white;text-align:center;padding:50px;">⏳ Загрузка данных... Обновите через 5 секунд.</h1><script>setTimeout(()=>location.reload(), 5000);</script>');
        ...
        linksHtml += `
            <div style="margin:10px 0;padding:15px;border:1px solid #333;background:#1e1e1e;border-radius:8px;">
                ...
                <div style="color:#8b949e;font-size:13px;margin-bottom:10px;">
                    👤 ${data.owner} | 📱 ${Object.keys(data.devices||{}).length}/${data.maxDevices || '∞'} | 
    ...
    <body>
        ...
        <div class="card">
            ...
            <form action="/generate" method="POST">
                <input type="text" name="name" placeholder="Название (опционально)">
            ...
            window.showDevices = function(id) {
                fetch('/devices/' + id).then(r => r.json()).then(devs => {
                    ...
                    for(const [uuid, d] of Object.entries(devs)) {
                        ...
                        html += '<b>' + (d.name||'Unknown') + '</b><br>IP: ' + d.ip + '<br>Status: ' + (d.active ? 'Active' : 'Inactive');
